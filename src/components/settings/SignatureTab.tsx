...
export function SignatureTab() {
  ...
  async function handleRevoke() {
    ...
    setMandate(null); setRevoking(false); setConfirmRevoke(false); showToast('Mandat revoque.')
  }
  ...
  async function handleCertUpload() {
    ...
    try {
      ...
      setHasCert(true); setCertFile(null); setKeyFile(null); setCertPwd(''); showToast('Certificat importe !')
    ...
    <div className="space-y-6">
        ...
        <div className="bg-[#2dd4a0]/5 border border-[#2dd4a0]/20 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              ['Accepte le', fmtDate(mandate.accepted_at)],
          ...
          <p className="text-xs text-gray-400">Vos factures sont signees automatiquement par Fatoura Pro en votre nom.</p>
          <button onClick={() => setConfirmRevoke(true)} className="border border-red-500/40 text-red-400 hover:bg-red-950/20 px-4 py-2 rounded-xl text-sm transition-colors">Revoquer le mandat</button>
        </div>
      )}
      ...
      {!mandate && hasCert && (
        <div className="bg-[#4a9eff]/5 border border-[#4a9eff]/20 rounded-2xl p-6 space-y-4">
          ...
          <p className="text-xs text-gray-400">Certificat importe. Vos factures sont signees avec votre propre certificat.</p>
        ...
        <div className="space-y-4">
          <div className="bg-red-950/20 border border-red-900/30 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-bold text-red-400">Signature non configuree</span>
            </div>
            <p className="text-xs text-gray-400">Sans signature, vos factures ne peuvent pas etre soumises a TTN/ElFatoora.</p>
          </div>
          ...
          <div className="bg-[#0f1118] border-2 border-[#d4a843]/40 rounded-2xl p-5 space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span></span>
                  <span className="text-sm font-bold text-white">Deleguer a Fatoura Pro</span>
                </div>
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#d4a843]/15 text-[#d4a843] border border-[#d4a843]/30 font-bold">RECOMMANDE  Economisez 300-400 TND/an</span>
              </div>
            </div>
            ...
            <ul className="space-y-1.5">
              {['Operationnel immediatement','Cachet renouvele automatiquement','Inclus dans votre abonnement','Legalement conforme (art. 4 politique TTN)'].map(a => (
            ...
            <div className="bg-[#161b27] rounded-xl p-4 border border-[#d4a843]/20">
              <p className="text-xs text-gray-400 mb-3">En acceptant ce mandat, vous autorisez Fatoura Pro a signer vos factures avec son sceau ANCE en votre nom. Revocable a tout moment.</p>
          ...
          <div className="bg-[#0f1118] border border-[#1a1b22] rounded-2xl p-5 space-y-4">
            ...
            <p className="text-xs text-gray-400">Necessite un achat aupres de l'ANCE (~300-400 TND/an) et une configuration technique.</p>
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs text-gray-500 uppercase tracking-wider mb-1.5">Cle privee (.pem ou .key)</label>
              ...
              <p className="text-[10px] text-gray-600">Le certificat est chiffre et stocke de facon securisee.</p>
      ...
      <ConfirmDialog open={confirmRevoke}
        ...
        description="Vous devrez configurer votre propre certificat ANCE pour continuer a soumettre des factures a TTN."
        confirmLabel="Oui, revoquer" dangerous loading={revoking}
